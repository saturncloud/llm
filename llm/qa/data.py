...
def data_path(dataset: str, model: Optional[str] = None) -> str:
    return os.path.join(DATASETS_DIR, dataset, f"{parse_model_name(model)}.jsonl" if model else DATA_FILE)


def index_path(dataset: str, model: str) -> str:
    return os.path.join(DATASETS_DIR, dataset, f"{parse_model_name(model)}.faiss")
